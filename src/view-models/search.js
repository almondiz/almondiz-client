...
export default class SearchViewModel {
  ...
  async searchFullTag(keyword) {
    StaticComponentRefs.toastRef.current?.error("searchFullTag API 없음");
    if ((keyword = filterText(keyword)) === "")   return [];
  ...
  async readSearchHistory() {
    StaticComponentRefs.toastRef.current?.error("readSearchHistory API 없음");
  ...
  async removeSearchHistory(idx) {
    StaticComponentRefs.toastRef.current?.error("removeSearchHistory API 없음");
  ...
  async searchShop(keyword) {
    StaticComponentRefs.toastRef.current?.error("searchShop API 없음");
    if ((keyword = filterText(keyword)) === "")   return [];
  ...
  async getPreferedLocationSet() {
    //StaticComponentRefs.toastRef.current?.error("getPreferedLocationSet API 없음");
  ...
  async setPreferedLocationSet(data) {
    StaticComponentRefs.toastRef.current?.error("setPreferedLocationSet API 없음");
  ...
  async getMyLocation() {
    StaticComponentRefs.toastRef.current?.error("getMyLocation API 없음");
